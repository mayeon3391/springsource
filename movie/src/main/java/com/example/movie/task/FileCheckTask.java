...
public class FileCheckTask {
    ...
    public void checkFile() {
        log.info("file Check Tesk...");

        // 데이터 베이스에서 어제 날짜 파일 목록 추출
        ...
        List<Path> fileListPaths = movieImageDTOs.stream()
                .map(dto -> Paths.get(uploadPath, dto.getImgURL(), dto.getUuid() + "_" + dto.getImgName()))
                .collect(Collectors.toList());

        movieImageDTOs.stream()
                .map(dto -> Paths.get(uploadPath, dto.getImgURL(), "s_" + dto.getUuid() + "_" + dto.getImgName()))
