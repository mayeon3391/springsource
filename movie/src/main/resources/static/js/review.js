...
const reviewList = () => {
  ...
  axios.get(`/reviews/${mno}/all`).then((res) => {
    ...
    let result = "";
    data.forEach((review) => {
      result += `<div class="d-flex justify-content-between py-2 border-bottom review-row" data-rno=${review.rno} data-emai=${review.email}>`;
      result += `<div class="flex-grow-1 align-self-center">`;
      result += `<div><span class="font-semibold">${review.text}</span></div>`;
      result += `<div class="small text-muted"><span class="d-inline-block me-3">${review.nickname}</span>`;
      result += `평점 : <span class="geade">${review.grade}</span><div class="starrr"></div></div>`;
